...
class Node:
    ...
    # looking for a better option to split
    ...
    def find_best_split(self, X, y, feature_subset):
        ...
        # features_subset number shows how many features should be selected
        if feature_subset is None:
            feature_subset = X.shape[1]

        # random selection
        selected_features = np.random.choice(X.shape[1], feature_subset, replace=False) # no repetition
